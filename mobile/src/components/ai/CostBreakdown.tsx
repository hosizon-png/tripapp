...
export function CostBreakdown({ items }: { items: { item: string; amount: string; note?: string }[] }) {
  if (!items?.length) return null;
  const total = items.reduce((s, i) => s + (parseInt(i.amount.replace(/[^0-9]/g, "")) || 0), 0);

  return (
    <View style={{ padding: 4 }}>
      <Text style={{ color: "#FFF", fontSize: 11, fontWeight: "600", letterSpacing: 0.5, marginBottom: 8 }}>💰 预估花费</Text>
      <Text style={{ color: "#FFF", fontSize: 28, fontWeight: "800", letterSpacing: -0.5 }}>¥{total}</Text>
      ...
      <View style={{ gap: 6, marginTop: 12 }}>
        {items.map((it, i) => (
          <View key={i} style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
            ...
            <Text style={{ color: "rgba(255,255,255,0.5)", fontSize: 11, flex: 2, textAlign: "right" }} numberOfLines={1}>{it.note || ""}</Text>
